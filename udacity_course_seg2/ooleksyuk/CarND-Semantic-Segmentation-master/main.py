...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        print("[Epoch: {0}/{1} Loss: Time: {2}]".format(str(epoch + 1), str(epochs), str(timedelta(seconds=(time.time() - s_time)))))
...
def run():
    ...
    with tf.Session(config=config) as sess:
        ...
        get_batches_fn = helper.gen_batch_function(os.path.join(DATA_DIR, '/home/shared/datasets/data_road/training'), IMAGE_SHAPE_KITI)
