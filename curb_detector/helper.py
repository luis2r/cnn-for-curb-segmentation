    ...
    def get_batches_fn(batch_size):
        ...
        for batch_i in range(0, len(image_paths), batch_size):
            ...
            for image_file in image_paths[batch_i:batch_i+batch_size]:
                ...
                #print("gt max",np.max(gt_images))
                #print("gt min",np.max(gt_images))
...
def gen_test_output(sess, logits, keep_prob, image_pl, data_folder, image_shape):
    ...
    for image_file in glob(os.path.join(data_folder, 'data', '*.png')):
        ...
        mask = np.dot(segmentation, np.array([[255, 0, 0, 100]]))
...
def save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image, shape_org):
